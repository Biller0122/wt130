...
router.post('/:id/import', auth, requireRole('ADMIN', 'MANAGER'), upload.single('file'), async (req, res) => {
  ...
  // Бүх сэлбэгийг системээс авна — агуулахад урьдчилан оноогоогүй байсан ч тохируулна
  const parts = await prisma.part.findMany()
  if (parts.length === 0) return res.status(400).json({ error: 'Системд сэлбэг бүртгэгдээгүй байна' })
  ...
  const system = `Та агуулахын тооллогын Excel файлыг дүн шинжилгээ хийж, системийн сэлбэгүүдтэй тохируулах үүрэгтэй.
...
- qty: Excel-ийн тоо хэмжээ (заавал тоо байна, текст биш)
Зөвхөн Excel-д тоо хэмжээтэй, системд байгаа сэлбэгүүдийг буцаана. Тохирохгүй мөрийг орхино.`

  const user = `Системийн сэлбэгийн каталог (Код | Нэр | Нэгж):
${partsList}
